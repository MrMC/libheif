...
func (img *Image) GetPlane(channel Channel) (*ImageAccess, error) {
	...
	pointer := unsafe.Pointer(plane)
	size := stride * height
	access := &ImageAccess{
		Plane:   C.GoBytes(pointer, size),
		Stride:  int(stride),
		Height:  int(height),
		Pointer: pointer,
		image:   img,
...
func hasAlpha(i image.Image) bool {
	rect := i.Bounds()
	for y := rect.Min.Y; y < rect.Max.Y; y++ {
		for x := rect.Min.X; x < rect.Max.X; x++ {
			if _, _, _, c := i.At(x, y).RGBA(); c == 0xffff {
				return true
			}
		}
	}
...
func imageFromYCbCr(i *image.YCbCr) (*Image, error) {
	...
	out, err := NewImage(w, h, ColorspaceYCbCr, Chroma420)
	if err != nil {
		return nil, fmt.Errorf("failed to create image: %v", err)
	}
...
func EncodeFromImage(img image.Image, compression Compression, quality int, lossless losslessMode, logging loggingLevel) (*Context, error) {
	...
	default:
		return nil, fmt.Errorf("color model %v not supported yet", cm)
